...
const StackedBarChart: React.FC<StackedBarChartProps> = ({
  ...
  function applyAggregationThreshold(items: DataItem[], threshold: number): DataItem[] {
    ...
    const belowThreshold = items.filter(item => 
      item.values.every(value => value < threshold)
    );
    ...
    const aboveThreshold = items.filter(item => 
      !item.values.every(value => value < threshold)
    );
    ...
    return [
      ...aboveThreshold,
      {
        ...
        category: activeTab === 'investments' ? 'investment' : activeTab === 'income' ? 'income' : 'expense',
  ...
  return (
    ...
    >
      <Tabs 
        ...
        mb={4}
        onChange={(index) => {
          const tabs: Array<'investments' | 'income' | 'expenses'> = ['investments', 'income', 'expenses'];
      ...
      <Text fontSize="sm" mt={2} color="gray.600" textAlign="center">
        {activeTab === 'investments' ? (
          <>
            This chart shows the {aggregationType} value of each investment over time.
            Investments are color-coded by tax status: 
            <Box as="span" color="#ff6666" fontWeight="bold"> Non-Retirement</Box>,
            <Box as="span" color="#3399ff" fontWeight="bold"> Pre-Tax</Box>, and
            <Box as="span" color="#33ff33" fontWeight="bold"> After-Tax</Box>.
          </>
        ) : activeTab === 'income' ? (
          <>
          ...
          <>
            This chart shows the {aggregationType} value of each expense category over time.
            Taxes are included as a separate category.
          </>
        )}
      </Text>
    </Box>
