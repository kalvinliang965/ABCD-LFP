...
function parse_life_expectancy(lifeExpectancy: Array<Map<string, any>> ): Array<number> {
    ...
    const parse = (params: Map<string, any>): number => {
        ...
        switch (params.get("type")) {
            case "fixed":
                ...
                if (!value) {
                    throw new Error(`life expectancy value field does not exist for fixed type: ${params}`);
                }
            ...
            case "normal":
                ...
                if (!mean) {
                    throw new Error(`life expectancy mean field does not exist for normal type: ${params}`);
                }
                const stdev = params.get("stdev");
                if (!stdev) {
                    throw new Error(`life expectancy stdev field does not exist for normal type: ${params}`);
                }
            ...
            default:
                throw new Error(`Invalid type for calculating life expectancy ${params}`);
        }
    }
    try {

        const user_life_expectancy = parse(lifeExpectancy[0]);
        const spouse_life_expectancy = (lifeExpectancy.length == 2)? parse(lifeExpectancy[1]): -1;
...
function Scenario(params: {
    ...
    try {
        ...
        const roth_conversion_strategy: Array<string> = params.RothConversionStrategy;
        ...
        throw new Error(`An error occured while creating Scenario instance: ${error}`);
    }
}
